...
class TestPaddle(init_config_class.InitConfigClass):
    ...
    def _cal_eager_res(self, x, table, dout):
        ...
        if self._dtype == "bfloat16":
            table_t = paddle.cast(table_t, dtype="uint16")
            dout_t = paddle.cast(dout, dtype="uint16")
        
        embedding = fleet.meta_parallel.VocabParallelEmbedding(init_config_class.dim_1, init_config_class.dim_3, weight_attr=paddle.fluid.initializer.NumpyArrayInitializer(self._np_paddle_table), mp_group=self._group)
        out = embedding(x_t)
            ...
            if paddle.distributed.get_rank() == 0:
                ...
                try: 
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Incubate: vocab_parallel_embedding eager forward is unstable in %s dtype'
                        )
                        % self._dtype,
                    )
                except Exception as e:
                ...
                try:
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Incubate: vocab_parallel_embedding eager grad is unstable in %s dtype'
                        )
                        % self._dtype,
                    )
                except Exception as e:
                ...
                if paddle.distributed.get_rank() == 0:
                    try:
                        np.testing.assert_equal(
                            ...
                            out_static_baseline,
                            err_msg=(
                                'Incubate: vocab_parallel_embedding static forward is unstable in %s dtype'
                            )
                            % self._dtype,
                        )
                    except Exception as e:
                    ...
                    try: 
                        np.testing.assert_equal(
                            ...
                            err_msg=(
                                'Incubate: vocab_parallel_embedding static grad is unstable in %s dtype'
                            )
                            % self._dtype,
                        )
                    except Exception as e:
