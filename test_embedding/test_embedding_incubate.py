...
class TestEmbeddingIncubateCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
    ...
    def init_np_inputs_and_dout(self):
        ...
        self.np_x = np_inputs_array["x"].astype(np.int64)
    ...
    def cal_eager_res(self, x, w, dout):
        if self.dtype == "bfloat16":
            w = paddle.cast(w, dtype="uint16")
            dout = paddle.cast(dout, dtype="uint16")
        out = paddle.nn.functional.embedding(x, w)
        out_grads = paddle.grad(
            [out], [w], grad_outputs=[dout]
        )
        if self.dtype == "bfloat16":
    ...
    def test_eager_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_eager_develop,
            err_msg=(
                'Incubate: compare embedding incubate eager forward res with develop eager forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_eager_np)):
            ...
            err_msg=(
                'Incubate: compare embedding incubate eager grad res with develop eager grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_static_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_static_develop,
            err_msg=(
                'Incubate: compare embedding incubate static forward res with develop static forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_static)):
            ...
            err_msg=(
                'Incubate: compare embedding incubate static grad res with develop static grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_eager_stability(self):
        ...
        for i in range(50):
            ...
            np.testing.assert_equal(
                ...
                err_msg=(
                    'Develop: paddle.nn.functional.embedding eager forward is unstable in %s dtype'
                )
                % self.dtype,
            )
            for idx in range(len(out_grads_eager)):
                np.testing.assert_equal(
                    ...
                    err_msg=(
                        'Develop: paddle.nn.functional.embedding eager grad is unstable in %s dtype'
                    )
                    % self.dtype,
                )
        ...
        with paddle.fluid.framework._dygraph_guard(None):
            ...
            for i in range(50):
                ...
                np.testing.assert_equal(
                    ...
                    out_static_baseline,
                    err_msg=(
                        'Develop: paddle.nn.functional.embedding static forward is unstable in %s dtype'
                    )
                    % self.dtype,
                )
                for idx in range(len(out_grads_static)):
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Develop: paddle.nn.functional.embedding static grad is unstable in %s dtype'
                        )
                        % self.dtype,
                    )


class TestEmbeddingIncubateCase1_FP16(TestEmbeddingIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.dtype = "float16"
...
class TestEmbeddingIncubateCase1_BFP16(TestEmbeddingIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.dtype = "bfloat16"
