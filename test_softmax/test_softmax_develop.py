...
def generate_np_inputs_and_dout():
    x_case = np.random.random(size=[1, 32, 4096, 4096]).astype("float32")
    dout_case = np.random.random(size=[1, 32, 4096, 4096]).astype("float32")

    np.savez("./inputs_case1.npz", x=x_case, dout=dout_case)


class TestSoftmaxDevelopCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
    ...
    def cal_torch_res(self, x, dout):
        ...
        out = torch.nn.functional.softmax(x_t, dim=self.axis)
    ...
    def cal_eager_res(self, x, dout):
        ...
        out = paddle.nn.functional.softmax(x, axis=self.axis)
    ...
    def test_eager_stability(self):
        ...
        for i in range(50):
            ...
            np.testing.assert_equal(
                ...
                err_msg=(
                    'Develop: paddle.nn.funtional.softmax eager forward is unstable in %s dtype'
                )
                % self.dtype,
            )
...
class TestSoftmaxDevelopCase1_FP16(TestSoftmaxDevelopCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "float16"
...
class TestSoftmaxDevelopCase1_BF16(TestSoftmaxDevelopCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "bfloat16"
