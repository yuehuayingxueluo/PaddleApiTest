...
class TestSoftmaxIncubateCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
    ...
    def cal_eager_res(self, x, dout):
        ...
        out = paddle.fluid.layers.softmax(x, axis=self.axis)
    ...
    def test_eager_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_eager_develop,
            err_msg=(
                'Incubate: compare paddle.fluid.layers.softmax incubate eager forward res with develop eager forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_eager_np)):
            ...
            err_msg=(
                'Incubate: compare paddle.fluid.layers.softmax incubate eager grad res with develop eager grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_static_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_static_develop,
            err_msg=(
                'Incubate: compare paddle.fluid.layers.softmax incubate static forward res with develop static forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_static)):
            ...
            err_msg=(
                'Incubate: compare paddle.fluid.layers.softmax incubate static grad res with develop static grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_eager_stability(self):
        ...
        for i in range(50):
            ...
            np.testing.assert_equal(
                ...
                err_msg=(
                    'Incubate: paddle.fluid.layers.softmax eager forward is unstable in %s dtype'
                )
                % self.dtype,
            )
            for idx in range(len(out_grads_eager)):
                np.testing.assert_equal(
                    ...
                    err_msg=(
                        'Incubate: paddle.fluid.layers.softmax eager grad is unstable in %s dtype'
                    )
                    % self.dtype,
                )
        ...
        with paddle.fluid.framework._dygraph_guard(None):
            ...
            for i in range(50):
                ...
                np.testing.assert_equal(
                    ...
                    out_static_baseline,
                    err_msg=(
                        'Incubate: paddle.fluid.layers.softmax static forward is unstable in %s dtype'
                    )
                    % self.dtype,
                )
                for idx in range(len(out_grads_static)):
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Incubate: paddle.fluid.layers.softmax static grad is unstable in %s dtype'
                        )
                        % self.dtype,
                    )


class TestSoftmaxIncubateCase1_FP16(TestSoftmaxIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "float16"
...
class TestSoftmaxIncubateCase1_BF16(TestSoftmaxIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "bfloat16"
