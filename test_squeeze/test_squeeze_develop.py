...
def generate_np_inputs_and_dout():
    x_case = np.random.random(size=[1, 4096]).astype("float32")
    dout_case = np.random.random(size=[1, 4096]).astype("float32")

    np.savez("./inputs_case1.npz", x=x_case, dout=dout_case)


class TestSqueezeDevelopCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
        self.init_np_inputs_and_dout()
        input_x, dout_torch = self.gen_torch_inputs_and_dout()
        out_torch, out_grads_torch = self.cal_torch_res(input_x, dout_torch)
        del input_x
        del dout_torch
    ...
    def cal_torch_res(self, x, dout):
        ...
        out = torch.squeeze(x_t, dim=self.axis)
    ...
    def cal_static_res(self, x, dout):
        if self.dtype == "bfloat16":
            ...
            dout = paddle.cast(dout, dtype="bfloat16")
        out = paddle.squeeze(x, axis=self.axis)
    ...
    def test_eager_accuracy(self):
        ...
        np.testing.assert_allclose(
            ...
            self.rtol,
            err_msg=(
                'Develop: compare paddle.squeeze eager forward res with torch failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_eager_np)):
            np.testing.assert_allclose(
                ...
                self.rtol,
                err_msg=(
                    'Develop: compare paddle.squeeze eager grad res with torch failed in %s dtype'
                )
                % self.dtype,
            )
    ...
    def test_static_accuracy(self):
        ...
        np.testing.assert_allclose(
            ...
            self.rtol,
            err_msg=(
                'Develop: compare paddle.squeeze static forward res with torch failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_static)):
            np.testing.assert_allclose(
                ...
                self.rtol,
                err_msg=(
                    'Develop: compare paddle.squeeze static grad res with torch failed in %s dtype'
                )
                % self.dtype,
            )
    ...
    def test_eager_stability(self):
        ...
        for i in range(50):
            ...
            np.testing.assert_equal(
                ...
                err_msg=(
                    'Develop: paddle.squeeze eager forward is unstable in %s dtype'
                )
                % self.dtype,
            )
            for idx in range(len(out_grads_eager)):
                np.testing.assert_equal(
                    ...
                    err_msg=(
                        'Develop: paddle.squeeze eager grad is unstable in %s dtype'
                    )
                    % self.dtype,
                )
        ...
        with paddle.fluid.framework._dygraph_guard(None):
            ...
            for i in range(50):
                ...
                np.testing.assert_equal(
                    ...
                    out_static_baseline,
                    err_msg=(
                        'Develop: paddle.squeeze static forward is unstable in %s dtype'
                    )
                    % self.dtype,
                )
                for idx in range(len(out_grads_static)):
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Develop: paddle.squeeze static grad is unstable in %s dtype'
                        )
                        % self.dtype,
                    )


class TestSqueezeDevelopCase1_FP16(TestSqueezeDevelopCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "float16"
...
class TestSqueezeDevelopCase1_BF16(TestSqueezeDevelopCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.axis = -1
        self.dtype = "bfloat16"
