...
class TestFlashAttentionDevelopCase1_FP16(unittest.TestCase):
    ...
    def init_np_inputs_and_dout(self):
        ...
        self.np_dout = np.random.random(size=[1,4096,16,128]).astype("float32") - 0.5
    ...
    def cal_torch_res(self, x, dout):
        ...
        batch_size, seqlen, num_head, head_dim = x.shape
        x_t = x.reshape((batch_size * seqlen, x.shape[2], x.shape[3]))

        if self.dtype == "bfloat16":
            x_t = x_t.to(dtype=torch.bfloat16)
            dout = dout.to(dtype=torch.bfloat16)
        
        cu_seqlens = torch.arange(0, (batch_size + 1) * seqlen, step=seqlen, dtype=torch.int32, device=x.device)
        out = flash_attn_unpadded_func(x_t, x_t, x_t, cu_seqlens, cu_seqlens, seqlen, seqlen, 0.0, causal=True)
        out = out.reshape((batch_size, seqlen, num_head, head_dim))

        out_grads = torch.autograd.grad([out], [x_t], grad_outputs=[dout])
        if self.dtype == "bfloat16":
