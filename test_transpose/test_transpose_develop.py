...
def generate_np_inputs_and_dout():
    ...
    dout_case1 = np.random.random(size=[1, 4096, 32, 192]).astype("float32")
...
class TestTransposeDevelopCase1_FP32(unittest.TestCase):
    ...
    def test_eager_accuracy(self):
        ...
        np.testing.assert_allclose(
            ...
            self.rtol,
            err_msg=(
                'Develop: compare transpose eager forward res with torch failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_eager_np)):
            np.testing.assert_allclose(
                ...
                self.rtol,
                err_msg=(
                    'Develop: compare transpose eager grad res with torch failed in %s dtype'
                )
                % self.dtype,
            )
    ...
    def test_static_accuracy(self):
        ...
        np.testing.assert_allclose(
            ...
            self.rtol,
            err_msg=(
                'Develop: compare transpose static forward res with torch failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_static)):
            np.testing.assert_allclose(
                ...
                self.rtol,
                err_msg=(
                    'Develop: compare transpose static grad res with torch failed in %s dtype'
                )
                % self.dtype,
            )
