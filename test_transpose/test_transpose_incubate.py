...
class TestTransposeIncubateCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
    ...
    def test_eager_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_eager_develop,
            err_msg=(
                'Incubate: compare transpose incubate eager forward res with develop eager forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_eager_np)):
            ...
            err_msg=(
                'Incubate: compare transpose incubate eager grad res with develop eager grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_static_accuracy(self):
        ...
        np.testing.assert_equal(
            ...
            out_static_develop,
            err_msg=(
                'Incubate: compare transpose incubate static forward res with develop static forward res failed in %s dtype'
            )
            % self.dtype,
        )
        for idx in range(len(out_grads_static)):
            ...
            err_msg=(
                'Incubate: compare transpose incubate static grad res with develop static grad res failed in %s dtype'
            )
                % self.dtype,
            )
    ...
    def test_eager_stability(self):
        ...
        for i in range(50):
            ...
            np.testing.assert_equal(
                ...
                err_msg=(
                    'Develop: paddle.transpose eager forward is unstable in %s dtype'
                )
                % self.dtype,
            )
            for idx in range(len(out_grads_eager)):
                np.testing.assert_equal(
                    ...
                    err_msg=(
                        'Develop: paddle.transpose eager grad is unstable in %s dtype'
                    )
                    % self.dtype,
                )
        ...
        with paddle.fluid.framework._dygraph_guard(None):
            ...
            for i in range(50):
                ...
                np.testing.assert_equal(
                    ...
                    out_static_baseline,
                    err_msg=(
                        'Develop: paddle.transpose static forward is unstable in %s dtype'
                    )
                    % self.dtype,
                )
                for idx in range(len(out_grads_static)):
                    np.testing.assert_equal(
                        ...
                        err_msg=(
                            'Develop: paddle.transpose static grad is unstable in %s dtype'
                        )
                        % self.dtype,
                    )


class TestTransposeIncubateCase1_FP16(TestTransposeIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.dtype = "float16"
...
class TestTransposeIncubateCase1_BFP16(TestTransposeIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case1.npz"
        self.dtype = "bfloat16"
...
class TestTransposeIncubateCase2_FP32(TestTransposeIncubateCase1_FP32):
    def init_params(self):
...
class TestTransposeIncubateCase2_FP16(TestTransposeIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case2.npz"
        self.dtype = "float16"
...
class TestTransposeIncubateCase2_BFP16(TestTransposeIncubateCase1_FP32):
    def init_params(self):
        self.np_input_dir = "./inputs_case2.npz"
        self.dtype = "bfloat16"
