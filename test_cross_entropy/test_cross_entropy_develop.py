...
class TestCrossEntropyDevelopCase1_FP32(unittest.TestCase):
    ...
    def cal_torch_res(self, logits, label, dout):
        ...
        out = torch.nn.functional.cross_entropy(logits_t, label_t, weight=None, size_average=None, ignore_index=-100, reduce=None, reduction='none', label_smoothing=0.0)
        out_grads = torch.autograd.grad([out], [logits_t], grad_outputs=[dout_t])
        if self.dtype == "bfloat16":
